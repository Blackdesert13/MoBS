...
namespace MoBaSteuerung {
	...
	public class ArduinoController : Form {
		...
		bool _displayTimeDuration = false;
		DateTime _lastRM;
		...
		private void GetBefehl() {
			while (this._receivedBytes.Count >= 5) {
				if (_receivedBytes[4] == (byte)((_receivedBytes[0] + _receivedBytes[1] + _receivedBytes[2] + _receivedBytes[3]) % 256)) {
					if(_receivedBytes[0]==1 && _receivedBytes[1] == 19) {
						_lastRM = DateTime.Now;
						_displayTimeDuration = true;
					}
			...
			if (IsPortOpen()) {
				if (data != null)
					try {
						...
						if (_displayTimeDuration) {
							_displayTimeDuration = false;
							TimeSpan dur = DateTime.Now - _lastRM;
							Debug.Print("Zeit seit RMempfang (ms):" + dur.Milliseconds);
